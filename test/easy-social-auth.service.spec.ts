...
describe("SocialAuthService", () => {
  ...
  it("should generate Google auth URL", () => {
    ...
    if (socialAuthService.googleStrategy) {
      const authUrl =
        socialAuthService.googleStrategy.generateAuthUrl("redirect_uri");
  ...
  it("should generate Facebook auth URL", () => {
    ...
    if (socialAuthService.facebookStrategy) {
      const authUrl =
        socialAuthService.facebookStrategy.generateAuthUrl("redirect_uri");
  ...
  it("should generate Twitter auth URL", () => {
    ...
    if (socialAuthService.twitterStrategy) {
      const authUrl =
        socialAuthService.twitterStrategy.generateAuthUrl("redirect_uri");
  ...
  it("should exchange code for Twitter token", async () => {
    ...
    if (socialAuthService.twitterStrategy) {
      ...
      mock
        .onPost(twitterConfig.tokenEndpoint)
        .reply(200, { access_token: mockToken });

      const response =
        await socialAuthService.twitterStrategy.exchangeCodeForToken(
          mockCode,
          "redirect_uri"
        );
      expect(response.status).toBe(true);
  ...
  it("should refresh Google access token", async () => {
    ...
    if (socialAuthService.googleStrategy) {
      ...
      mock
        .onPost(googleConfig.tokenEndpoint)
        .reply(200, { access_token: mockNewToken });

      const response =
        await socialAuthService.googleStrategy.refreshAccessToken(
          mockRefreshToken
        );
      expect(response.status).toBe(true);
  ...
  it("should refresh Facebook access token", async () => {
    ...
    if (socialAuthService.facebookStrategy) {
      ...
      const response =
        await socialAuthService.facebookStrategy.refreshAccessToken(
          mockRefreshToken
        );
      expect(response.status).toBe(true);
      expect(response.data).toEqual(mockNewToken);
  ...
  it("should refresh Twitter access token", async () => {
    ...
    if (socialAuthService.twitterStrategy) {
      ...
      const response =
        await socialAuthService.twitterStrategy.refreshAccessToken(
          mockRefreshToken
        );
      expect(response.status).toBe(true);
      expect(response.data).toBeDefined();
  ...
  it("should get Google user data", async () => {
    ...
    if (socialAuthService.googleStrategy) {
      ...
      const response = await socialAuthService.googleStrategy.getUserData(
        mockToken
      );
      expect(response.status).toBe(true);
      expect(response.data).toBeDefined();
  ...
  it("should get Facebook user data", async () => {
    ...
    if (socialAuthService.facebookStrategy) {
      ...
      const mockUserData = {
        ...
        first_name: "Test",
        last_name: "User",
        picture: { data: { url: "picture_url" } },
      };
      mock.onGet(facebookConfig.userInfoEndpoint).reply(200, mockUserData);

      const response = await socialAuthService.facebookStrategy.getUserData(
        mockToken
      );
      expect(response.status).toBe(true);
      expect(response.data).toBeDefined();
